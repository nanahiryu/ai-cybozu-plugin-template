...
コードを書く前に、まず落ちるテスト（Red）を書きます。
これにより、AI が満たすべき要件がコード（テストコード）として明確になります。

### 8. 実装計画の構築 (GitHub Issue)
